 ...
 * $config['vacation_log'] = true; // To activate logs
 *
 */
...
function vacation_read(array &$data)
{
  ...
  rcube::write_log('errors', "Response: " . $response);
  ...
  // Set id
  $userid = $decoded[0]->id;
  rcube::write_log('errors', "$userid: " . $userid);
...
function vacation_write(array &$data)
{
  ...
  rcube::write_log('errors', "encoded write: " . $encoded);
  ...
  $response = curl_exec($curl);
  rcube::write_log('errors', "response write: " . $response);
  $err = curl_error($curl);
  rcube::write_log('errors', "Curl error: " . $err);
